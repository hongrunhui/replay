...
const checkpoint_pool_js_1 = require("./checkpoint-pool.js");
...
class ReplaySession {
    ...
    // Create engine object without starting a process (for runToLine which does its own start)
    ensureEngine() {
        if (this._engine)
            return;
        this._engine = new replay_engine_js_1.ReplayEngine({ recordingPath: this._recordingPath });
    ...
    get checkpointPool() {
        if (!this._checkpointPool) {
            this._checkpointPool = new checkpoint_pool_js_1.CheckpointPool(this._recordingPath);
        }
